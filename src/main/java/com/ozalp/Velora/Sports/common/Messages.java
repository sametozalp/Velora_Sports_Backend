package com.ozalp.Velora.Sports.common;

public class Messages {

    public static class AuthMessages {

        public static final String PASSWORDS_NOT_MATCHED = "Passwords not matched";
    }

    public static class RefreshToken {
        public static final String NOT_FOUND = "Refresh token not found";
        public static final String INVALID_TOKEN = "Invalid token";
    }

    public static class AthleteProgress {
        public static final String NOT_MATCHED = "User and athlete progress not matched";
        public static final String NOT_FOUND = "Athlete progress not found";

    }

    public static class AdminMessages {
        public static final String NOT_FOUND = "Admin not found";
    }

    public static class AthleteMessages {
        public static final String NOT_FOUND = "Athlete not found";

    }

    public static class CoachMessages {
        public static final String NOT_FOUND = "Coach not found";

    }

    public static class ExerciseMessages {
        public static final String NOT_FOUND = "Exercise not found";

    }

    public static class MembershipMessages {
        public static final String NOT_FOUND = "Membership not found";

    }

    public static class OrganizationMessages {
        public static final String NOT_FOUND = "Organization not found";

    }

    public static class RoleMessages {
        public static final String NOT_FOUND = "Role not found";

    }

    public static class UserMessages {
        public static final String NOT_FOUND = "User not found";

    }

    public static class UserRoleMessages {
        public static final String NOT_FOUND = "User role not found";

    }

    public static class WorkoutItemMessages {
        public static final String NOT_FOUND = "Workout item not found";

    }

    public static class WorkoutProgramMessages {
        public static final String NOT_FOUND = "Workout program not found";

    }
}
